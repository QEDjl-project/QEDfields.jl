...
@testset "pulse envelope" begin
    ...
    @testset "compute single" begin
        ...
        @test isapprox(
            pulse_envelope(test_field, rnd_phi),
            _groundtruth_envelope(rnd_phi),
            atol=ATOL,
            rtol=RTOL,
        )
        @test isapprox(
            pulse_envelope(test_field, leftendpoint(RND_DOMAIN)),
            _groundtruth_envelope(leftendpoint(RND_DOMAIN)),
            atol=ATOL,
            rtol=RTOL,
        )
        @test isapprox(
            pulse_envelope(test_field, leftendpoint(RND_DOMAIN) - eps()),
            zero(Float64),
            atol=ATOL,
            rtol=RTOL,
        )
        @test isapprox(
            pulse_envelope(test_field, rightendpoint(RND_DOMAIN)),
            _groundtruth_envelope(rightendpoint(RND_DOMAIN)),
            atol=ATOL,
            rtol=RTOL,
        )
        @test isapprox(
            pulse_envelope(test_field, rightendpoint(RND_DOMAIN) + eps()),
            zero(Float64),
            atol=ATOL,
            rtol=RTOL,
        )
